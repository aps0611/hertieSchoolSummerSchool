...
animalsSelected <- c(houseMouse, giraffe, wildBoar)
animalsSelected
...
animalsSelected > 4.0 & animalsSelected < 25
```

```{r}
animalsSelected == 4 | animalsSelected == 29
...
```{r}
animalString <- c('houseMouse','giraffe','housefly')
animalString
...
animalLifespan <- c(houseMouse, giraffe, wildBoar)
animalLifespan
...
conversion <- animalLifespan/human_lifespan
conversion
```
...
```{r}
giraffe_to_human_years <- function(animal_years){

  human_lifespan <- 122.5
  giraffe_lifespan <- 39.5

  ratio <- human_lifespan/giraffe_lifespan
